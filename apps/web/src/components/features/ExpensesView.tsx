  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <motion.div 
          whileHover={{ y: -5 }}
          className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-xl relative overflow-hidden flex flex-col justify-center gap-6 group"
        >
          <div className="absolute right-0 top-0 p-8 opacity-5 text-emerald-600 transition-transform duration-700 group-hover:scale-110 group-hover:rotate-12"><PieChart size={160} /></div>
          <div>
            <div className="flex justify-between items-end mb-4">
              <span className="text-[10px] font-black uppercase text-emerald-600 tracking-[0.2em] bg-emerald-50 px-4 py-2 rounded-2xl border border-emerald-100">Pagado Total</span>
              <span className="text-sm font-black text-emerald-600">
                {financeStats.total > 0 ? Math.round((financeStats.paid / financeStats.total) * 100) : 0}%
              </span>
            </div>
            <p className="text-5xl font-black text-slate-800 tracking-tighter">S/ {financeStats.paid.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</p>
          </div>
          <div className="w-full bg-slate-100 h-4 rounded-full overflow-hidden border border-slate-50 p-1">
            <motion.div 
              ...
              className="bg-emerald-500 h-full rounded-full transition-all duration-1000 ease-out shadow-lg shadow-emerald-200"
            />
          </div>
        </motion.div>
        ...
        <motion.div 
          whileHover={{ y: -5 }}
          className="premium-gradient p-10 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col justify-center gap-6 group"
        >
      ...
      <div className="flex flex-col md:flex-row justify-between items-center px-4 gap-6">
        <h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-4">
          <div className="bg-emerald-100 p-2.5 rounded-2xl text-emerald-600"><DollarSign size={24} /></div>
          Detalle de Gastos
        </h2>
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="bg-emerald-600 hover:bg-emerald-700 shadow-xl shadow-emerald-100 text-white px-8 py-4 rounded-3xl transition-all active:scale-95 flex items-center gap-3 font-black text-xs uppercase tracking-widest"
          >
      ...
      {/* Search Bar & Filters Ultra-Optimized (Copied Style from GuestsView) */}
      ...
      <DataTable 
        columns={[
          { header: 'Concepto', className: 'w-1/3' },
          { header: 'Progreso Pago', className: 'w-1/4' },
          { header: 'Costo Total', className: 'text-center w-40' },
          { header: 'Estado', className: 'text-center w-32' },
      ...
      >
        {filteredExpenses.map((g) => {
          ...
          return (
            <tr key={g.id} className="hover:bg-slate-50/40 transition-all group">
              <td className="px-8 py-8">
                <p className="font-black text-slate-800 text-sm mb-1 uppercase tracking-tight">{g.item}</p>
                <div className="flex items-center gap-2 mb-4">
                  <span className="px-2 py-1 rounded-lg bg-slate-100 text-[9px] font-bold text-slate-500 uppercase tracking-widest border border-slate-200">
                    {g.categoria}
                  </span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {g.responsable?.map((r) => (
                    <div key={r} className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-sm">
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
                      <span className="text-[9px] font-black uppercase text-slate-500">{r}:</span>
                      <span className="text-[9px] font-black text-slate-800">S/ {pagos[r] || 0}</span>
                    </div>
                  ))}
                </div>
              </td>
              <td className="px-6 py-8 align-middle">
                <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden mb-3 border border-slate-100">
                  <motion.div 
                    initial={{ width: 0 }}
                ...
                <div className="flex justify-between text-[10px] font-black uppercase tracking-wider">
                  <span className="text-emerald-600">S/ {paidAmount.toFixed(2)}</span>
                  <span className="text-slate-400">S/ {Math.max(0, costo - paidAmount).toFixed(2)}</span>
                </div>
              </td>
              <td className="px-6 py-8 text-center">
                <span className="text-lg font-black text-slate-800">S/ {costo.toFixed(2)}</span>
              </td>
              <td className="px-6 py-8 text-center">
                <span className={`inline-block px-4 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest border whitespace-nowrap ${
                  status === 'Pagado' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 
                  status === 'En Proceso' ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 
              ...
              <td className="px-8 py-8 text-right">
                ...
                <div className="flex justify-end gap-2">
                  <button 
                    ...
                    className="p-3 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all outline-none focus:ring-2 focus:ring-indigo-500 bg-white border border-slate-100 shadow-sm"
                  >
                    <Pencil size={18} />
                  </button>
                  <button 
                    ...
                    className="p-3 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-2xl transition-all outline-none focus:ring-2 focus:ring-rose-500 bg-white border border-slate-100 shadow-sm"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
